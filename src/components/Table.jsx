...
const TableCard=({orderId,successful,transactionID,refundDate,orderAmount})=>
{
    return (
        <tr className=' border-b'>
            ...
            <td className='text-text py-3.5 px-3 space-x-1.5 text-sm leading-5 font-medium flex items-center'>
                <aside className={`w-2.5 h-2.5 ${successful?"bg-green-600":"bg-gray-500"} rounded-full`}></aside>
  ...
  return (
    <div className="bg-white border border-white  rounded-lg p-3 space-y-3" style={{boxShadow: "0px 2px 6px 0px rgba(26, 24, 30, 0.04)"}}>
        ...
        <table className=' w-full text-left border-collapse '>
            ...
            {Data.map(({orderId,successful,transactionID,refundDate,orderAmount})=>(
                <TableCard key={orderId} orderId={281209} successful={successful} transactionID={transactionID} refundDate={refundDate} orderAmount={orderAmount} />
            ))}
            </table>
          </div>
  )
